...
class Client:
    ...
    def try_login(self, request:dict) -> None:
        ...
        if 'username' not in request or 'password' not in request:
            self.connection.send({'request':'login', 'return':False, 'error':'Missing username or password'})
                
        success, error_message = self.database.validate_user(request['username'], request['password'])
        if not success:
            self.connection.send({'request': 'login', 'return':False, 'error':error_message})
    ...
    def create_account(self, request:dict) -> None:
        ...
        if 'username' not in request or 'password' not in request or 'initials' not in request:
            self.connection.send({'request':'login', 'return':False, 'error':'Missing Information'})
                
        success = self.database.create_user(request['username'], request['password'], request['initials'])
        if not success:
            self.connection.send({'request': 'login', 'return':False, 'error':'Failed to create account'})
    ...
    def run_thread(self) -> None:
        ...
        while not self.shut_down.is_set():
            ...
            if not self.authenticated:
                self.connection.send({'request':request['request'], 'return':False, 'error':'Not Authorized'})
                continue
            else:
