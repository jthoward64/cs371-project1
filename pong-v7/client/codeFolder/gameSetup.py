...
class gameInstance:
    def __init__(
        ...
        self.scoreFont = pygame.font.Font(
            path.join(assets_dir, "fonts", "pong-score.ttf"), 32
        )
        self.winFont = pygame.font.Font(
            path.join(assets_dir, "fonts", "visitor.ttf"), 48
        )
        self.pointSound = pygame.mixer.Sound(
            path.join(assets_dir, "sounds", "point.wav")
        )
        self.bounceSound = pygame.mixer.Sound(
            path.join(assets_dir, "sounds", "bounce.wav")
        )
        # Display objects
        ...
        self.leftPaddle = Paddle(
            pygame.Rect(10, self.paddleStartPosY, self.paddleWidth, self.paddleHeight)
        )
        self.rightPaddle = Paddle(
            pygame.Rect(
                screenWidth - 20,
                self.paddleStartPosY,
                self.paddleWidth,
                self.paddleHeight,
            )
        )
    ...
    def reset(self):
        ...
        self.ball = Ball(
            pygame.Rect(self.screenWidth / 2, self.screenHeight / 2, 5, 5), -5, 0
        )
        self.isBuilt = 1
