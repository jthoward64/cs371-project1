...
from code.settings import LOWER_PORT, MAIN_PORT, UPPER_PORT

# Server and Port Information
from code.sockethelper import Client, Connection, EncodeMessage
...
class Lobby:
    ...
    def client_handler(self, socket: SSLSocket) -> None:
        ...
        while not self.shut_down.is_set() and not close_client:
            ...
            assert isinstance(
                new_message, dict
            ), f"Expected dict, got {type(new_message)}"
            ...
            if new_message["request"] == "login":
                ...
                if validated:
                    ...
                    connection.send(
                        EncodeMessage(
                            {"request": "login", "return": True, "message": "Success"}
                        )
                    )
                    continue
            ...
            if new_message["request"] == "create_account":
                validated = database.create_user(
                    new_message["username"],
                    new_message["password"],
                    new_message["initials"],
                )
                if validated:
                    logged_in = True
                    connection.send(
            ...
            if new_message["request"] == "create_game":
                ...
                new_game = th.Thread(
                    target=self.game_handler, args=(new_code, new_port)
                )
                new_game.start()
                            ...
                            EncodeMessage(
                                {
                                    "request": "join_game",
                                    "return": True,
                                    "message": self.game_codes[new_message["code"]],
                                }
                            )
                        )
                        continue
